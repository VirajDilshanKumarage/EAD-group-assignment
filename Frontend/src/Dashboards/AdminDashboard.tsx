// AdminDashboard.jsx
import React, { useEffect, useState } from 'react';
import Card from './Card'; // Create a Card component or use a library like Material-UI for ready-made components
import settingIcon from '../Asset/setting.png'; // Import your setting icon image
import logoutIcon from '../Asset/logout.png'; // Import your logout icon image
import { useLocation, useNavigate } from 'react-router-dom';
import Modal from './UpdateProfileModal';
import image from '../Asset/customer.jpg';


const AdminDashboard = () => {

  type NullableType = any | null;
  
  const [name, setName] = useState('');
  const [role, setRole] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [adminId, setUserId] = useState(0);
  const [adminData, setAdminData] = useState(null as NullableType);
  const location = useLocation();


  useEffect(() => {
    const loginId = location.state?.userId || '';
    setUserId(loginId);

    // Fetch customer data when the component mounts
    const fetchCustomerData = async () => {
      try {
        const userDataResponse = await fetch(`http://localhost:8080/api/v1/users/getUserById/${loginId}`);
      
        if (userDataResponse.ok) {
          const userData = await userDataResponse.json();
          
  
           setAdminData(userData);
           setName(userData.name);
           setRole(userData.role);
           console.log('User admin Data-name:', name);
         
          
        } else {
          // Handle error when retrieving user data
          console.error('Failed to retrieve user data:', userDataResponse.statusText);
        }
      } catch (error) {
        console.error('Error fetching customer data:', error);
      }
    };

    fetchCustomerData();
  }, [location.state]);

  

   

  useEffect(() => {
    // Set email only once when the component mounts
    const loginId = location.state?.userId ||'';
    setUserId(loginId);
  }, [location.state]);

  const openModal = () => {
    setIsModalOpen(true);
  };

  const closeModal = () => {
    setIsModalOpen(false);
  };
  

  const navigate = useNavigate();

  const goToLoginPage = () => {
    navigate('/');
  };

  const goToCustomerDetailsPage = () => {
    navigate('/customer-Details');
  };

  const goToEmployeeDetailsPage = () => {
    navigate('/employee-Details');
  };

  const goToInventoryPage= () => {
    navigate('/inventory');
  };

  const goToDeliveryPage= () => {
    navigate('/delivery');
  };
  
  

  

  return (
    <div className="admin-dashboard">
      <h1>Welcome to Admin Dashboard</h1>
      
      <div className="main-content">
        <div className='back-ground1'>
        <Modal isOpen={isModalOpen} onClose={closeModal} userLoginId={adminId} />
          <div className='profile'>
            <div className='profile-image'>
            <img src="data:image/jpeg;base64," alt="Admin" className="admin-image" />
            </div>
            <div className="profile-info">

           
              <h3>{role}</h3>
              <h3>{name}</h3>
            </div>
            <div className='button-container'>
              <button className='setting-button' onClick={openModal} >
                <img src={settingIcon} alt="Settings" className='setting-icon' />
                Setting
              </button>
              <button className='logout-button' onClick={goToLoginPage}>
                <img src={logoutIcon} alt="Log Out" className='logout-icon' />
                Logout
              </button>
            </div>
          </div>

          

          <div className="card-container">
          <Card
              title="Customers"
              icon="👥"
              backgroundImage='https://www.ibm.com/blog/wp-content/uploads/2021/07/Grocery-Store-Experience.png' // Replace with your image path
              onClick={goToCustomerDetailsPage}
              
            >

            </Card>
            <Card 
                title="Employees" 
                icon="👔"  
                backgroundImage="https://www.adb.org/sites/default/files/content-media/146416-211207-supermarket-image.jpeg" 
                onClick={goToEmployeeDetailsPage} >
              {/* Employee-related content */}
              <div>
                
              </div>
            </Card>
            <Card 
                  title="Inventory" 
                  icon="📦"  
                  backgroundImage="https://apprissretail.com/wp-content/uploads/sites/4/2023/03/grocery-store-man-stocking-shelves-inventory-500px.webp" 
                  onClick={goToInventoryPage}>
              {/* Inventory-related content */}
            </Card>
            <Card 
                    title="Deliveries" 
                    icon="🚚"  
                    backgroundImage="https://archives1.dailynews.lk/sites/default/files/news/2020/06/16/pg10-SRL-1.jpg" 
                    onClick={goToDeliveryPage}>
              {/* Delivery-related content */}
            </Card>

          </div>
        </div>
      </div>
    </div>
  );
};

export default AdminDashboard;
